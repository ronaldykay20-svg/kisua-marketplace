...
const isStoryActive = (createdAt: string) => {
  ...
  return diffHours < STORY_ACTIVE_HOURS;
};
...
const SellerStoriesTab = ({ sellerId }: Props) => {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    if (file.size > 50 * 1024 * 1024) { toast.error("Vídeo demasiado grande (máx. 50MB)"); return; }
    try {
      const duration = await checkVideoDuration(file);
      if (duration > MAX_DURATION_SECONDS) {
        toast.error(`Vídeo tem ${Math.round(duration)}s. Máximo: 2 minutos.`);
        if (fileInputRef.current) fileInputRef.current.value = "";
        return;
      }
    } catch { toast.error("Não foi possível verificar a duração."); return; }
  ...
  const openRepublish = (story: any) => {
    if (isStoryActive(story.created_at)) { toast.error("Este story ainda está activo (menos de 24h)."); return; }
    if (totalStoriesCount >= MAX_STORIES) { toast.error(`Limite de ${MAX_STORIES} stories atingido. Apaga antigos primeiro.`); return; }
  ...
  const deleteStory = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["seller_stories", sellerId] }); toast.success("Story removido!"); },
    onError: (e: any) => toast.error(e.message),
  });

  const resetForm = () => {
    setStep("idle"); setSelectedFile(null); setPreviewUrl(null); setLinkedProductId("");
    if (fileInputRef.current) fileInputRef.current.value = "";
  };
    ...
    <div className="space-y-4">
      ...
      <div className="bg-card rounded-2xl border border-border overflow-hidden">
        ...
        <div className="flex items-center px-4 pt-3 pb-1">
          {[{ n: 1, label: "Vídeo" }, { n: 2, label: "Produto" }, { n: 3, label: "Publicar" }].map((s, i) => {
            const done = (s.n === 1 && ["file_selected","choosing_product","uploading"].includes(step)) ||
                         (s.n === 2 && ["choosing_product","uploading"].includes(step)) ||
                         (s.n === 3 && step === "uploading");
            const active = (s.n === 1 && step === "idle") || (s.n === 2 && step === "file_selected") || (s.n === 3 && step === "choosing_product");
            return (
              <div key={s.n} className="flex items-center">
                <div className="flex flex-col items-center">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold transition-all ${done ? "bg-primary text-primary-foreground" : active ? "bg-primary/20 text-primary border border-primary" : "bg-muted text-muted-foreground"}`}>
        ...
        <div className="px-4 pb-4 pt-2 space-y-3">
          {step === "idle" && (
            <>
              <input ref={fileInputRef} type="file" accept="video/*" className="hidden" onChange={handleFileSelect} disabled={atLimit} />
              <button
                onClick={() => !atLimit && fileInputRef.current?.click()}
                disabled={atLimit}
                className={`w-full border-2 border-dashed rounded-xl py-8 flex flex-col items-center gap-2 transition-all group ${atLimit ? "border-border opacity-40 cursor-not-allowed" : "border-border hover:border-primary hover:bg-primary/5 cursor-pointer"}`}
              >
                ...
                <p className="text-sm font-bold text-foreground">{atLimit ? "Limite atingido" : "Selecionar Vídeo"}</p>
                <p className="text-[10px] text-muted-foreground">{atLimit ? "Apaga stories antigos primeiro" : "Toca para escolher da galeria"}</p>
              </button>
            </>
          )}
          ...
          {step === "file_selected" && previewUrl && (
            <div className="space-y-3">
              <div className="relative rounded-xl overflow-hidden bg-black aspect-[9/16] max-h-48 flex items-center justify-center">
                ...
                <button onClick={resetForm} className="absolute top-2 right-2 w-6 h-6 rounded-full bg-black/60 flex items-center justify-center text-white text-xs">✕</button>
              </div>
              <p className="text-[10px] text-muted-foreground truncate">📹 {selectedFile?.name}</p>
              <button onClick={() => setStep("choosing_product")} className="w-full flex items-center justify-center gap-2 py-2.5 bg-primary text-primary-foreground text-sm font-bold rounded-xl">
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="flex items-center gap-1.5 text-xs font-bold text-foreground mb-1.5">
                  <Link2 className="w-3.5 h-3.5 text-primary" /> Vincular a produto (opcional)
                </label>
                <select value={linkedProductId} onChange={e => setLinkedProductId(e.target.value)}
                  ...
                  {products.map((p: any) => <option key={p.id} value={p.id}>{p.title} — {Number(p.price).toLocaleString("pt-AO")} Kz</option>)}
                </select>
              </div>
              <div className="flex gap-2">
                <button onClick={() => setStep("file_selected")} className="flex-1 py-2.5 border border-border rounded-xl text-sm font-bold text-muted-foreground">Voltar</button>
                <button onClick={handlePublish} disabled={uploading} className="flex-1 py-2.5 bg-primary text-primary-foreground text-sm font-bold rounded-xl disabled:opacity-60 flex items-center justify-center gap-2">
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 pb-6 px-4">
          <div className="bg-card rounded-2xl border border-border w-full max-w-sm overflow-hidden">
            <div className="px-4 pt-4 pb-3 border-b border-border">
              ...
              <p className="text-[10px] text-muted-foreground mt-0.5">Podes alterar o produto antes de republicar.</p>
            </div>
            {republishStoryData.image_url && (
              <div className="px-4 pt-3">
                <div className="rounded-xl overflow-hidden bg-black aspect-[9/16] max-h-32">
                  <VideoThumbnail src={republishStoryData.image_url} />
                </div>
              </div>
            )}
            <div className="px-4 py-3 space-y-3">
              <div>
                <label className="flex items-center gap-1.5 text-xs font-bold text-foreground mb-1.5">
                  <Link2 className="w-3.5 h-3.5 text-primary" /> Produto vinculado (opcional)
                </label>
                <select value={republishProductId} onChange={e => setRepublishProductId(e.target.value)}
                  className="w-full border border-border rounded-lg px-3 py-2 text-sm bg-background text-foreground focus:outline-none focus:border-primary">
                  <option value="">Nenhum produto</option>
                  {products.map((p: any) => <option key={p.id} value={p.id}>{p.title} — {Number(p.price).toLocaleString("pt-AO")} Kz</option>)}
                </select>
              ...
              <div className="flex gap-2">
                <button onClick={() => setRepublishStoryData(null)} className="flex-1 py-2.5 border border-border rounded-xl text-sm font-bold text-muted-foreground">Cancelar</button>
                <button onClick={handleRepublish} disabled={republishing} className="flex-1 py-2.5 bg-primary text-primary-foreground text-sm font-bold rounded-xl disabled:opacity-60 flex items-center justify-center gap-2">
                  {republishing ? <div className="w-4 h-4 rounded-full border-2 border-white border-t-transparent animate-spin" /> : <><RefreshCw className="w-4 h-4" /> Republicar</>}
                </button>
              </div>
            </div>
          </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-8 text-muted-foreground text-sm">Nenhum story publicado. Publique o primeiro!</div>
      ) : (
