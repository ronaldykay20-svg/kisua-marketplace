    ...
    <section className="container mx-auto px-3 pt-3">
      <div className={`grid gap-2.5 ${hasSinglePromo ? "grid-cols-1" : "grid-cols-2"}`}>
        {cards.map((card) => (
          <div
            key={card.title}
            className={`rounded-gpu-fix relative rounded-card overflow-hidden cursor-pointer group ${hasSinglePromo ? "min-h-[240px] sm:min-h-[320px]" : "min-h-[200px] sm:min-h-[260px]"}`}
          >
