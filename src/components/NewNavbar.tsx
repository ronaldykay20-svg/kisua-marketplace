...
import { Gavel, Monitor, Home, Car, Watch, Clock, Trophy, CheckCircle2, Users, Shield, Loader2, X, Upload, Copy, ChevronLeft, ChevronRight, Play } from "lucide-react";
...
const HeroCarousel = () => {
  ...
  // Fallback se não há slides configurados
  if (slides.length === 0) {
    return (
  ...
  return (
    <div className="relative w-full overflow-hidden" style={{ minHeight: 220 }}>
      ...
      {slides.length > 1 && (
        <>
          <button onClick={prev} className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/40 text-white flex items-center justify-center">
            <ChevronLeft className="w-5 h-5" />
          </button>
          <button onClick={next} className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/40 text-white flex items-center justify-center">
          ...
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
            {slides.map((_: any, i: number) => (
              <button key={i} onClick={() => setCurrent(i)}
                ...
                style={{ background: i === current ? "#f5c842" : "rgba(255,255,255,0.5)" }}
              />
            ))}
          </div>
        </>
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { error: insertError } = await (supabase as any).from("auction_bid_proofs").insert({
        auction_id: auction.id, user_id: user.id, amount,
        proof_url: path, reference: reference || null, status: "pending",
      });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div className="bg-card border border-border rounded-2xl p-5 w-[90vw] max-w-sm shadow-xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-2">
              {methods.map((m: any) => (
                <div key={m.id} className="bg-muted rounded-xl px-3 py-2 flex items-center justify-between">
                  ...
                  <button onClick={() => { navigator.clipboard.writeText(m.value); toast.success("Copiado!"); }}
                    className="p-1.5 rounded-lg" style={{ background: "rgba(200,169,126,0.2)", color: "#a07a4a" }}>
        ...
        <input type="text" placeholder="Referência da transferência (opcional)" value={reference}
...
const Leilao = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {bidTarget && (
        <BidModal auction={bidTarget} onClose={() => setBidTarget(null)}
          onConfirm={(amount) => { setProofTarget({ auction: bidTarget, amount }); setBidTarget(null); }} />
      )}
      {proofTarget && (
        <ProofModal auction={proofTarget.auction} amount={proofTarget.amount}
          onClose={() => setProofTarget(null)} onSuccess={() => setProofTarget(null)} />
      )}

      {/* ── Hero carrossel ── */}
      <HeroCarousel />

      {/* ── Countdown do leilão em destaque ── */}
      {displayed && (
        ...
        <section className="container mx-auto px-3 mt-4">
          <div className="rounded-2xl overflow-hidden shadow-lg border border-border">
            ...
            <div className="p-4 bg-card">
              ...
              <button onClick={() => openBidModal(displayed)}
              ...
              <div className="mt-4">
                ...
                <div className="space-y-1.5">
                  ...
                  {bids.map((b: any, idx: number) => (
                    <div key={b.id} className={`flex items-center justify-between text-xs px-3 py-2 rounded-xl ${idx === 0 ? "border border-amber-300" : "bg-muted/50"}`}
      ...
      {/* ── Leilões em andamento — cards novos ── */}
        ...
        <section className="container mx-auto px-3 mt-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {filteredActive.map((a: any) => (
              <div key={a.id} onClick={() => setSelectedAuction(a)}
                ...
                <div className="p-3">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); openBidModal(a); }}
