...
import {
  ...
  Send, Loader2, ShieldCheck, X, Building2, Link2, Search,
} from "lucide-react";
...
const MinimalProductCard = ({ product, onClick }: { product: any; onClick?: () => void }) => (
  <div onClick={onClick} className="cursor-pointer group flex flex-col flex-shrink-0" style={{ width: 148 }}>
    ...
    <div className="mt-1.5 px-0.5">
      <p className="text-xs font-semibold leading-snug line-clamp-2" style={{ color: N.brown }}>{product.title}</p>
...
const ShareSheet = ({ title, imageUrl, url, description, price, onClose }: { title: string; imageUrl: string; url: string; description?: string; price?: string; onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-end justify-center" style={{ background: "rgba(0,0,0,0.55)", backdropFilter: "blur(3px)" }} onClick={onClose}>
      <div className="w-full max-w-md rounded-t-3xl pb-8 pt-2 px-4 shadow-2xl" style={{ background: N.cream, border: `1.5px solid ${N.sand}`, borderBottom: "none" }} onClick={e => e.stopPropagation()}>
        <div className="w-10 h-1 rounded-full mx-auto mb-4 mt-2" style={{ background: N.sandDark }} />
        <p className="text-[11px] font-bold uppercase tracking-widest mb-3" style={{ color: N.sandDark }}>Partilhar produto</p>
        <div className="flex items-center gap-3 p-3 rounded-2xl mb-5" style={{ background: "#f5ede4", border: `1px solid ${N.sand}` }}>
          <div className="w-16 h-16 rounded-xl overflow-hidden flex-shrink-0"><img src={imageUrl} alt={title} className="w-full h-full object-cover" /></div>
          <div className="min-w-0 flex-1">
            <p className="text-sm font-bold leading-snug line-clamp-2" style={{ color: N.brown }}>{title}</p>
            {price && <p className="text-xs font-black mt-0.5" style={{ color: "#c0522a" }}>{price}</p>}
          </div>
        </div>
        <div className="grid grid-cols-3 gap-2.5 mb-3">
          <button onClick={handleNative} disabled={sharing} className="flex flex-col items-center gap-2 py-3.5 rounded-2xl active:scale-95 transition-all" style={{ background: N.brown }}>
          ...
          <button onClick={handleCopyLink} disabled={copying} className="flex flex-col items-center gap-2 py-3.5 rounded-2xl active:scale-95 transition-all" style={{ background: N.brownLight, border: `1.5px solid ${N.sand}` }}>
            {copying ? <Loader2 className="w-5 h-5 animate-spin" style={{ color: N.brown }} /> : <Link2 className="w-5 h-5" style={{ color: N.brown }} />}
            <span className="text-[10px] font-bold" style={{ color: N.brown }}>Copiar link</span>
          </button>
        </div>
        <button onClick={onClose} className="w-full py-3 rounded-2xl text-sm font-bold" style={{ background: N.brownLight, color: N.sandDark }}>Cancelar</button>
      </div>
    </div>
  );
};
...
const ProductDetail = () => {
  ...
  if (!productBase) return (
    <div className="min-h-screen flex items-center justify-center text-center px-6" style={{ background: N.cream }}>
      <div>
        <h2 className="text-lg font-bold mb-2" style={{ color: N.brown }}>Produto não encontrado</h2>
  ...
  const VariantPill = ({ v, selected, onSelect, type }: { v: any; selected: boolean; onSelect: () => void; type: string }) => {
    if (type === "color" && v.value?.startsWith("#")) {
      return (
        <button onClick={onSelect} className="relative rounded-xl overflow-hidden transition-all" style={{ border: selected ? `2.5px solid ${N.brown}` : `2px solid ${N.sand}`, width: 56, height: 56 }} title={v.name}>
          {v.image_url ? <img src={v.image_url} alt={v.name} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center"><div className="w-8 h-8 rounded-full border border-white/40" style={{ background: v.value }} /></div>}
    ...
    return (
      <button onClick={onSelect} className="px-3 py-2 rounded-xl text-xs font-bold transition-all" style={{ background: selected ? N.brown : N.brownLight, color: selected ? "#fff" : N.brown, border: selected ? `1.5px solid ${N.brown}` : `1.5px solid ${N.sand}` }}>
        {v.name}
  ...
  return (
    <div className="min-h-screen" style={{ background: N.cream }}>
        ...
        <div className="md:grid md:grid-cols-[64px_1fr_320px] md:gap-6">
          ...
          <div className="hidden md:flex flex-col gap-2 pt-2">
            {displayImages.map((img, i) => (
              <button key={i} onClick={() => setSelectedImage(i)} className="w-14 h-14 rounded-xl overflow-hidden flex-shrink-0 transition-all"
                style={{ border: i === selectedImage ? `2.5px solid ${N.brown}` : `2px solid ${N.sand}`, opacity: i === selectedImage ? 1 : 0.6 }}>
          ...
          <div>
            <div className="relative overflow-hidden md:rounded-2xl" style={{ background: "#f0e4d8" }}
              ...
              {product.badge === "HOT" && !product.discount && <div className="absolute top-3 left-3 px-2 py-1 rounded-lg text-xs font-black text-white" style={{ background: N.brown }}>🔥 Hot</div>}
              <button onClick={handleZoom} className="absolute bottom-3 right-3 w-9 h-9 rounded-full flex items-center justify-center" style={{ background: "rgba(255,255,255,0.85)", border: `1px solid ${N.sand}` }}>
                <ZoomIn className="w-4 h-4" style={{ color: N.brown }} />
              </button>
            </div>
              ...
              <div className="flex gap-2 px-4 mt-3 overflow-x-auto scrollbar-hide md:hidden">
                {displayImages.map((img, i) => (
                  <button key={i} onClick={() => setSelectedImage(i)} className="flex-shrink-0 w-14 h-14 rounded-xl overflow-hidden transition-all"
                    style={{ border: i === selectedImage ? `2.5px solid ${N.brown}` : `2px solid ${N.sand}`, opacity: i === selectedImage ? 1 : 0.65 }}>
                    {img.type === "video" ? <video src={img.url} className="w-full h-full object-cover" /> : <img src={img.url} alt="" className="w-full h-full object-cover" />}
                  </button>
                ))}
              </div>
          ...
          <div className="px-4 md:px-0 mt-4 md:mt-0">
            ...
            {(publisher || loadingPublisher) && (
              <div className="mb-2">
                {loadingPublisher
                  ? <div className="h-3 w-28 rounded animate-pulse" style={{ background: N.sand }} />
                  : publisher && (
                    <button onClick={handlePublisherNavigate} className="flex items-center gap-2 group">
                      ...
                      <div>
                        <span className="text-xs font-bold group-hover:underline" style={{ color: N.brown }}>{publisher.name}</span>
                        {publisher.is_verified && <ShieldCheck className="w-3 h-3 text-blue-500 inline ml-1" />}
                        {publisher.__type === "company" && <span className="ml-1 text-[9px] font-bold text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded-full border border-blue-100">Empresa</span>}
                        {province && <p className="text-[10px] flex items-center gap-0.5 mt-0.5" style={{ color: N.sandDark }}><MapPin className="w-2.5 h-2.5" />{province}</p>}
                      </div>
                      <ChevronRight className="w-4 h-4 ml-auto" style={{ color: N.sandDark }} />
                    </button>
                  )
                }
              </div>
            ...
            {product.rating && (
              <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                <div className="flex items-center gap-0.5">
                  {Array.from({ length: 5 }).map((_, i) => <Star key={i} className={`w-3.5 h-3.5 ${i < Math.floor(product.rating!) ? "fill-amber-400 text-amber-400" : "text-[#ddd]"}`} />)}
                </div>
                <span className="text-xs font-semibold" style={{ color: N.brown }}>{product.rating}</span>
                <span className="text-xs" style={{ color: N.sandDark }}>({product.reviews?.toLocaleString()} avaliações)</span>
            ...
            <div>
              ...
              {product.oldPrice && <span className="text-sm line-through" style={{ color: N.sandDark }}>De: {product.oldPrice}</span>}
              {product.freeShipping && <div className="flex items-center gap-1 mt-1 text-xs font-bold" style={{ color: "#1a5c3a" }}><Truck className="w-3.5 h-3.5" /> Frete grátis para Luanda</div>}
            </div>
            ...
            {Object.keys(variantGroups).length > 0 && (
              <div className="space-y-3">
                {Object.entries(variantGroups).map(([type, variants]) => {
                  ...
                  return (
                    <div key={type}>
                      <p className="text-sm font-bold mb-2" style={{ color: N.brown }}>{typeLabels[type] || type}{selId && <span className="font-normal ml-1" style={{ color: N.sandDark }}>: {variants.find((v: any) => v.id === selId)?.name}</span>}</p>
                ...
                <div className="h-px" style={{ background: N.sand }} />
              </div>
            )}
            ...
            <div className="hidden md:block space-y-3 mt-4">
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center rounded-xl overflow-hidden" style={{ border: `1.5px solid ${N.sand}` }}>
                  ...
                  <span className="w-10 text-center text-sm font-bold" style={{ color: N.brown }}>{qty}</span>
              ...
              <button onClick={handleAddToCart} disabled={addToCart.isPending} className="w-full py-3 rounded-2xl font-bold text-sm transition flex items-center justify-center gap-2 disabled:opacity-50" style={{ border: `2px solid ${N.brown}`, color: N.brown, background: "transparent" }}>
                {addToCart.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <ShoppingCart className="w-4 h-4" />} Adicionar ao carrinho
              </button>
              <button onClick={handleBuyNow} disabled={addToCart.isPending} className="w-full py-3 rounded-2xl font-bold text-sm text-white transition flex items-center justify-center gap-2 disabled:opacity-50 hover:brightness-110" style={{ background: `linear-gradient(135deg, ${N.brown} 0%, #2d1505 100%)` }}>
            ...
            <div className="space-y-3 mt-4 md:mt-0">
              {[
                ...
                { icon: <Shield className="w-3.5 h-3.5" style={{ color: "#1a5c3a" }} />, bg: "#e8f4ef", title: "Devolução grátis", sub: "Até 30 dias após a compra" },
                { icon: <ShieldCheck className="w-3.5 h-3.5 text-blue-500" />, bg: "#f0e8ff", title: "Pagamento seguro", sub: "Transação encriptada e protegida" },
              ].map((item, i) => (
                <div key={i} className="flex items-start gap-3 text-xs">
                  <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5" style={{ background: item.bg }}>{item.icon}</div>
                  <div><p className="font-semibold" style={{ color: N.brown }}>{item.title}</p><p className="mt-0.5" style={{ color: N.sandDark }}>{item.sub}</p></div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="space-y-2 mt-2 md:mt-6">
          ...
          <section className="px-4 py-5 md:rounded-2xl" style={{ background: "#fffaf6", border: `1px solid ${N.sand}` }}>
            ...
            {product.description && product.description.length > 280 && (
              <button onClick={() => setDescExpanded(v => !v)} className="mt-2 text-xs font-bold" style={{ color: N.brown }}>
                {descExpanded ? "Ver menos ▲" : "Ver mais ▼"}
              </button>
            )}
            <ul className="mt-4 space-y-1.5 text-xs" style={{ color: N.sandDark }}>
          ...
          {specRows.length > 0 && (
            <section className="px-4 py-5 md:rounded-2xl" style={{ background: "#fffaf6", border: `1px solid ${N.sand}` }}>
              ...
              <table className="w-full text-sm">
                <tbody>
                  {specRows.map((row, i) => (
                    <tr key={row.label} style={{ borderTop: i > 0 ? `1px solid ${N.sand}` : "none" }}>
                      <td className="py-2.5 pr-4 font-semibold w-2/5" style={{ color: N.sandDark }}>{row.label}</td>
                      <td className="py-2.5" style={{ color: N.brown }}>{row.value}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {productAds.length > 0 && (
            <section className="px-4 py-4 md:rounded-2xl space-y-3" style={{ background: "#fffaf6", border: `1px solid ${N.sand}` }}>
              ...
              {productAds.map((ad: any) => {
                const inner = (
                  <div className="rounded-2xl overflow-hidden hover:shadow-md transition-all group" style={{ border: `1px solid ${N.sand}` }}>
                    {ad.media_url ? (
                      <div className="relative">
                        {ad.media_type === "video" ? <video src={ad.media_url} className="w-full object-cover max-h-36" autoPlay muted loop playsInline /> : <img src={ad.media_url} alt={ad.title || ""} className="w-full object-cover max-h-36 group-hover:scale-[1.01] transition-transform" />}
                      ...
                      <div className="px-4 py-3 flex items-center justify-between gap-3" style={{ background: N.brownLight }}>
                        <p className="text-sm font-bold truncate" style={{ color: N.brown }}>{ad.title}</p>
                        <span className="text-[10px] font-bold border rounded-full px-2 py-0.5" style={{ color: N.brown, borderColor: N.sand }}>Ver mais</span>
                      </div>
                    )}
                  </div>
                );
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden"
        style={{
          ...
          borderTop: `1px solid ${N.sand}`,
          boxShadow: "0 -4px 20px rgba(74,46,10,0.14)",
        ...
        <div className="flex gap-2 px-3 pt-1">
          <button onClick={handleAddToCart} disabled={addToCart.isPending}
            className="flex-1 py-3 rounded-2xl font-bold text-sm transition flex items-center justify-center gap-1.5 disabled:opacity-50"
            style={{ border: `2px solid ${N.brown}`, color: N.brown, background: "transparent" }}>
          ...
          <button onClick={handleBuyNow} disabled={addToCart.isPending}
            className="flex-1 py-3 rounded-2xl font-bold text-sm text-white transition flex items-center justify-center gap-1.5 disabled:opacity-50 hover:brightness-110"
            style={{ background: `linear-gradient(135deg, ${N.brown} 0%, #2d1505 100%)` }}>
            {addToCart.isPending && <Loader2 className="w-4 h-4 animate-spin" />} Comprar agora
          </button>
        </div>
      </div>
    ...
    <section className="px-4 py-5 md:rounded-2xl" style={{ background: "#fffaf6", border: `1px solid ${N.sand}` }}>
      ...
      <div className="flex items-start gap-6 mb-5 flex-wrap">
        <div className="flex flex-col items-center">
          ...
          <span className="text-xs mt-1" style={{ color: N.sandDark }}>{totalReviews} avaliações</span>
        </div>
        {totalReviews > 0 && (
          <div className="flex-1 space-y-1.5 min-w-[150px]">
            {ratingCounts.map(({ star, count }) => (
              <div key={star} className="flex items-center gap-2">
                <span className="text-xs w-3 text-right" style={{ color: N.sandDark }}>{star}</span>
                <Star className="w-3 h-3 fill-amber-400 text-amber-400 flex-shrink-0" />
                <div className="flex-1 h-2 rounded-full overflow-hidden" style={{ background: N.sand }}>
                  <div className="h-full rounded-full" style={{ background: N.brown, width: `${totalReviews > 0 ? (count / totalReviews) * 100 : 0}%` }} />
                </div>
                <span className="text-xs w-4" style={{ color: N.sandDark }}>{count}</span>
              </div>
            ))}
          </div>
        )}
      ...
      {canReview && (
        <button onClick={() => setShowForm(!showForm)} className="mb-4 px-4 py-2 rounded-2xl text-xs font-bold text-white" style={{ background: N.brown }}>
          Avaliar produto
        </button>
      )}

      {showForm && canReview && (
        <div className="rounded-2xl p-4 mb-4" style={{ background: N.brownLight, border: `1.5px solid ${N.sand}` }}>
          <p className="text-sm font-bold mb-3" style={{ color: N.brown }}>A sua avaliação</p>
          <div className="flex items-center gap-1 mb-3">
            ...
            <span className="text-sm ml-2" style={{ color: N.sandDark }}>{reviewRating}/5</span>
          </div>
          <textarea value={reviewComment} onChange={e => setReviewComment(e.target.value)} placeholder="Escreva a sua opinião (opcional)..." rows={3}
            className="w-full px-3 py-2 rounded-xl text-sm resize-none focus:outline-none" style={{ background: "#fff", border: `1.5px solid ${N.sand}`, color: N.brown }} />
          <div className="mt-3">
            {reviewImage ? (
              <div className="relative w-24 h-24 rounded-xl overflow-hidden" style={{ border: `1.5px solid ${N.sand}` }}>
              ...
              <label className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold cursor-pointer" style={{ background: "#fff", border: `1.5px solid ${N.sand}`, color: N.brown }}>
          ...
          <div className="flex justify-end gap-2 mt-3">
            <button onClick={() => setShowForm(false)} className="px-4 py-2 rounded-2xl text-xs font-bold" style={{ color: N.sandDark }}>Cancelar</button>
            <button onClick={() => submitReview.mutate()} disabled={submitReview.isPending} className="px-4 py-2 rounded-2xl text-xs font-bold text-white disabled:opacity-50 flex items-center gap-1" style={{ background: N.brown }}>
      ...
      {alreadyReviewed && <p className="text-xs italic mb-3" style={{ color: N.sandDark }}>✓ Já avaliou este produto</p>}

      {reviews ? (
        <div className="space-y-5">
          {reviews.map((review: any) => (
            <div key={review.id} className="pt-4" style={{ borderTop: `1px solid ${N.sand}` }}>
              <div className="flex items-center gap-2 mb-2">
                {review.profile?.avatar_url
                  ...
                  : <div className="w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0" style={{ background: N.brownLight, color: N.brown }}>{(review.profile?.full_name || "U").charAt(0).toUpperCase()}</div>
                }
                <div>
                  <p className="text-xs font-bold" style={{ color: N.brown }}>{review.profile?.full_name || "Utilizador"}</p>
                  <div className="flex items-center gap-0.5 mt-0.5">
                    {Array.from({ length: 5 }).map((_, j) => <Star key={j} className={`w-3 h-3 ${j < review.rating ? "fill-amber-400 text-amber-400" : "text-[#ddd]"}`} />)}
                    <span className="text-[10px] ml-1" style={{ color: N.sandDark }}>{new Date(review.created_at).toLocaleDateString("pt-AO")}</span>
                  </div>
                </div>
              </div>
              {review.comment && <p className="text-sm leading-relaxed" style={{ color: N.brown }}>{review.comment}</p>}
              {review.image_url && <a href={review.image_url} target="_blank" rel="noopener noreferrer" className="block mt-2"><img src={review.image_url} alt="Foto da avaliação" className="max-h-40 rounded-xl object-cover" style={{ border: `1px solid ${N.sand}` }} /></a>}
              {review.replies?.length > 0 && (
                <div className="ml-8 mt-2 space-y-2">
                  {review.replies.map((reply: any) => (
                    <div key={reply.id} className="rounded-xl p-2.5" style={{ background: N.brownLight }}>
                      <p className="text-[11px] font-bold" style={{ color: N.brown }}>{reply.profile?.full_name || "Utilizador"}</p>
                      <p className="text-xs mt-0.5" style={{ color: N.sandDark }}>{reply.content}</p>
                    </div>
                  ))}
                </div>
              )}
              {user && (
                <button onClick={() => setReplyingTo(replyingTo === review.id ? null : review.id)} className="flex items-center gap-1 text-[11px] mt-2" style={{ color: N.sandDark }}>
              ...
              {replyingTo === review.id && user && (
                <div className="ml-8 mt-2 flex gap-2">
                  <input value={replyText} onChange={e => setReplyText(e.target.value)} placeholder="Escrever resposta..."
                    className="flex-1 px-3 py-1.5 rounded-xl text-xs focus:outline-none" style={{ background: "#fff", border: `1.5px solid ${N.sand}`, color: N.brown }} />
                  <button onClick={() => submitReply.mutate(review.id)} disabled={!replyText.trim() || submitReply.isPending}
                    className="px-3 py-1.5 rounded-xl text-xs font-bold text-white disabled:opacity-50" style={{ background: N.brown }}>
        ...
        <div className="text-center py-8" style={{ borderTop: `1px solid ${N.sand}` }}>
          <MessageCircle className="w-8 h-8 mx-auto mb-2" style={{ color: N.sand }} />
          <p className="text-sm font-semibold" style={{ color: N.brown }}>Ainda sem avaliações</p>
          <p className="text-xs mt-1" style={{ color: N.sandDark }}>Seja o primeiro a avaliar após a compra.</p>
        </div>
      )}
    </section>
  );
